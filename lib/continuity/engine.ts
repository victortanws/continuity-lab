 ...
 * source domain. `possible_prompt_injection` is deliberately a coarse,
 * chunk-level quarantine flag: flagged evidence may still be shown as context,
 * but `citationUseAllowed` prevents it from establishing or challenging truth.
 * We do not claim that every flagged span is malicious, and we do not silently
 * delete any part of the source.
 *
 * The directed checks require both a model/evaluation audience and a protected
 * operation. This keeps ordinary dialogue such as “ignore the old captain's
 * orders” from being mistaken for an instruction to the continuity system.
 */
export function detectEvidenceFlags(text: string, existing: string[] = []): string[] {
  const flags = new Set(existing);
  const normalized = text.replace(/[\u0000-\u001f\u007f]+/g, " ").replace(/\s+/g, " ").trim();

  // Classic boundary/control attacks remain high-signal even when they omit an
  // explicit audience such as “model” or “reviewer”. A system-prompt reference
  // is also quarantined because it concerns evaluator internals, not corpus
  // truth (including declarative requirements about revealing that prompt).
  const classicControlAttempt = /\b(?:ignore\s+(?:all|any|the)\s+(?:previous|prior|system)\s+instructions|disregard\s+(?:earlier|prior)\s+(?:directions|instructions)|follow\s+(?:these|the)\s+new\s+(?:rules|instructions)|you\s+are\s+now|system\s+prompt)\b/i;

  // Audience wording is intentionally imperative or output-scoped. Merely
  // narrating that a fictional reviewer reviewed evidence is not enough.
  ...
  const convenientIdentity = /\b(?:choose|pick|select|resolve|merge|collapse|treat)\b.{0,160}\b(?:identity|identities|referent|entity|entities|candidate|name|character)\b.{0,100}\b(?:convenient|easiest|simplest|whichever|first\s+available|makes?\s+the\s+answer\s+work)\b/i
    .test(normalized)
  ...
  if (
    classicControlAttempt.test(normalized)
    || (directedAtEvaluator && (
