...
export function buildContinuityInstructions(): string {
  return [
    ...
    "When trusted_reachability is present, copy its status, blockers, assumptions, and server-owned path faithfully. When it is absent, do not claim a deterministic reachable or unreachable path; use unknown or not_evaluated and explain the evidentiary dependencies instead.",
    ...
    "Coverage closure describes the material answer, not the strongest individual source. One complete registry does not close an answer when an upstream identity, provenance, input, or causal bridge remains outside that registry. Use closed only when every material boundary is trustworthy and complete; partial only when an authoritative source enumerates a bounded known omission; and open when a material unknown has no trustworthy completeness boundary.",
